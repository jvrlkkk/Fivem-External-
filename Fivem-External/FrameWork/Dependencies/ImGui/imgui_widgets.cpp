...
bool ImGui::ButtonText(const char* label, const ImVec2& size_arg)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    ImVec2 pos = window->DC.CursorPos;
    if ((ImGuiButtonFlags_None & ImGuiButtonFlags_AlignTextBaseLine) && style.FramePadding.y < window->DC.CurrLineTextBaseOffset) // Try to vertically align buttons that are smaller/have no padding so that text baseline matches (bit hacky, since it shouldn't be a flag)
        pos.y += window->DC.CurrLineTextBaseOffset - style.FramePadding.y;
...
bool ImGui::Tab(const char* Label, const char* Icon, bool Enabled)
{
    ...
    it_anim->second.a = ImLerp(it_anim->second.a, Enabled ? ImColor(232, 63, 212) : ImColor(232, 63, 212, 0), g.IO.DeltaTime * 10.f);
    it_anim->second.b = ImLerp(it_anim->second.b, hovered ? ImColor(232, 63, 212, 50) : ImColor(232, 63, 212, 0), g.IO.DeltaTime * 10.f);
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    it_anim->second.color = ImLerp(it_anim->second.color, *v ? ImColor(232, 63, 212) : ImColor(14, 14, 14), g.IO.DeltaTime * 10.f);
    it_anim->second.b = ImLerp(it_anim->second.b, hovered ? ImColor(232, 63, 212, 50).Value : ImColor(ImGui::GetColorU32(ImGuiCol_Border)).Value, g.IO.DeltaTime * 10.f);
...
bool ImGui::KeyBind(const char* label, int* k)
{
    ...
    const ImRect total_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(window->Size.x - 20, 23 ));
    ...
    window->DrawList->AddText(ImVec2(total_bb.Min.x, frame_bb.Min.y + frame_bb.GetHeight() / 2 - CalcTextSize(label).y / 2), ImColor(255, 255, 255), label);
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags, int items_count)
{
    ...
    const ImRect rect(window->DC.CursorPos + ImVec2(0, 1), window->DC.CursorPos + ImVec2(size, 45));
    const ImRect clickable(window->DC.CursorPos + ImVec2(-1, 20), window->DC.CursorPos + ImVec2(size, 45));
    ...
    window->DrawList->AddRectFilled(clickable.Min, clickable.Max, ImColor(14, 14, 14), 4);
    ...
    window->DrawList->AddCircleFilled(rect.Max - ImVec2(11, 13.f), 3.f + it_anim->second.active_circle_anim, ImColor(232, 63, 212));
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect frame_bb(window->DC.CursorPos + ImVec2(0, label_size.y + 2), window->DC.CursorPos + ImVec2(GetWindowSize().x - 20, 18 + label_size.y + 2));
    ...
    static std::map<ImGuiID, pumfedido> anim;
    auto it_anim = anim.find(id);

    if (it_anim == anim.end())
    {
        anim.insert({ id, pumfedido() });
        it_anim = anim.find(id);
    }


    GetWindowDrawList()->AddRectFilled(frame_bb.Min, frame_bb.Max, ImColor(14, 14, 14), 5);
    ...
    it_anim->second.a = ImLerp(it_anim->second.a, grab_bb.Min.x - frame_bb.Min.x, g.IO.DeltaTime * 10.f);
    it_anim->second.b = ImLerp(it_anim->second.b, hovered ? ImColor(232, 63, 212, 50).Value : ImColor(ImGui::GetColorU32(ImGuiCol_Border)).Value, g.IO.DeltaTime * 10.f);
    ...
    float rounding = value < 5.f ? 20 : 5;

    if (value > 1)
        window->DrawList->AddRectFilled(frame_bb.Min + ImVec2(1, 1), frame_bb.Min + ImVec2(it_anim->second.a, grab_bb.Max.y - grab_bb.Min.y + 3), ImColor(232, 63, 212), rounding);
    ...
    GetWindowDrawList()->AddText(frame_bb.GetCenter() - TextSize / 2 - ImVec2(0, 1), ImColor(255, 255, 255), value_buf, value_buf_end);

    PopFont();

    if (label_size.x > 0.0f)
        RenderText(ImVec2(total_bb.Min.x + 1, total_bb.Min.y), label);
...
static ImVec2 InputTextCalcTextSizeW(ImGuiContext* ctx, const ImWchar* text_begin, const ImWchar* text_end, const ImWchar** remaining, ImVec2* out_offset, bool stop_on_new_line)
{
    ...
    const float scale = line_height / 14;
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    const ImGuiID id = window->GetID(label);
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImVec2 frame_size = ImVec2(GetWindowSize().x - 20 - 20 - CalcTextSize(label).x, 20); // Arbitrary default of 8 lines high for multi-line
    const ImVec2 total_size = ImVec2(frame_size.x, frame_size.y);

    const ImRect frame_bb(window->DC.CursorPos + ImVec2(GetWindowSize().x - 20 - frame_size.x, 5 ), window->DC.CursorPos + ImVec2(GetWindowSize().x - 20, 30));
    const ImRect total_bb(window->DC.CursorPos, frame_bb.Max);
    ...
    it_anim->second.rect_color = ImLerp(it_anim->second.rect_color, hovered ? ImColor(232, 63, 212, 50).Value : ImColor(ImGui::GetColorU32(ImGuiCol_Border)).Value, g.IO.DeltaTime * 10.f);
    ...
    if (render_cursor || render_selection)
    {
...

        {
            ...
            if (searches_result_line_no[1] >= 0)
            {
                ...
                select_start_offset.y = searches_result_line_no[1] * 14;
            }

            // Store text height (note that we haven't calculated text width at all, see GitHub issues #383, #1224)
            if (is_multiline)
                text_size = ImVec2(inner_size.x, line_count * 14);
        }
        ...
        if (render_selection)
        {
            ...
            {
                if (rect_pos.y > clip_rect.w + 14.f)
                    break;
                ...
                else
                {
                    ...
                    if (rect.Overlaps(clip_rect))
                        draw_window->DrawList->AddRectFilled(rect.Min + ImVec2(0,3), rect.Max, bg_color, 3);
                }
        ...
        if (is_multiline || (buf_display_end - buf_display) < buf_display_max_length)
        {
            ...
            draw_window->DrawList->AddText(FrameWork::Assets::InterRegular14, 14, draw_pos - draw_scroll, col, buf_display, buf_display_end, 0.0f, is_multiline ? NULL : &clip_rect);
        }
        ...
        if (render_cursor)
        {
            ...
            if (cursor_is_visible && cursor_screen_rect.Overlaps(clip_rect))
                draw_window->DrawList->AddLine(frame_bb.Min + ImVec2(it_anim->second.value, 5), frame_bb.Min + ImVec2(it_anim->second.value, 20), GetColorU32(ImGuiCol_Text));
    ...
    else
    {
        ...
        {
            ImU32 col = GetColorU32(is_displaying_hint ? ImGuiCol_TextDisabled : ImGuiCol_Text);
            draw_window->DrawList->AddText(FrameWork::Assets::InterRegular14, 14, draw_pos, col, buf_display, buf_display_end, 0.0f, is_multiline ? NULL : &clip_rect);
        }
    }
    ...
    if (label_size.x > 0)
        RenderText(ImVec2(total_bb.Min.x + style.ItemInnerSpacing.x, frame_bb.Min.y + style.FramePadding.y), label);
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    if (label != label_display_end && !(flags & ImGuiColorEditFlags_NoLabel))
    {
        ...
        GetWindowDrawList()->AddText(GetWindowPos() + GetCursorPos() + ImVec2(0, square_sz / 2 - CalcTextSize(label).y / 2 - window->Scroll.y), ImColor(255, 255, 255), label);
    }
    ...
    {
        window->DC.CursorPos = ImVec2(pos.x + GetWindowSize().x - 20 - square_sz, pos.y);
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, const ImVec2& size_arg)
{
    ...
    else
    {
        ...
        it_anim->second.a = ImLerp(it_anim->second.a, hovered ? ImColor(232, 63, 212, 50).Value : ImColor(ImGui::GetColorU32(ImGuiCol_Border)).Value, g.IO.DeltaTime * 10.f);

        window->DrawList->AddRect(bb_inner.Min - ImVec2(1, 1), bb_inner.Max + ImVec2(1, 1), GetColorU32(it_anim->second.a), rounding, 0, 1);
...
void ImGui::ColorTooltip(const char* text, const float* col, ImGuiColorEditFlags flags)
{
    ...
    if (!BeginTooltipEx(ImGuiTooltipFlags_OverridePrevious, ImGuiWindowFlags_None, text, ImGui::IsItemHovered()))
        return;
